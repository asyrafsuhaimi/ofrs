  ...
  <table class="center">
    ...
    if(mysqli_num_rows($result_status)>0)
    {
      while($status_result = mysqli_fetch_array($result_status)){
        ...
        echo "<tr><td>".$facilityname."</td><td>".$bookingdatefrom."</td><td>".$bookingdateto."</td><td>".$bookingtime."</td><td>".$status."</td><td><form method='post' action='../delete.php'><button type='submit' value='".$reservationid."' name='delete' onclick=\"javascript: return confirm('Please confirm deletion');\">Cancel Reservation</button></td></tr>";

      }
    }
    else if(mysqli_num_rows($result_status1)>0) {
